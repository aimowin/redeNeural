...
int main(){
	...
	for(int x=0;x<1000;x++){
		...
		if(img<imagens/2){
			if(amostra[img]>bias)circulocerto++;
			if(amostra[img]<bias)quadradoerrado++;
		}else{
			if(amostra[img]<bias)quadradocerto++;
			if(amostra[img]>bias)circuloerrado++;
		}
	}
